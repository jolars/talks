...
\date{March 24, 2025}
\titlegraphic{\hfill\includegraphics[height=2cm]{figures/logo.pdf}\hspace{1cm}\includegraphics[height=2cm]{figures/ucph-vertical.pdf}\bigskip}

\usepackage[style=authoryear,url=false,backend=biber]{biblatex}
\addbibresource{bibliography.bib}

\title{Optimization and Algorithms in Sparse Regression}
\subtitle{Cramér Mini-Conference 2025}
\author{Johan Larsson\\\smallskip\scriptsize \url{https://jolars.co}, {\texttt{@jolars@mastodon.social}}}
\institute{Department of Mathematical Sciences, University of Copenhagen\\Department of Statistics, Lund University}
...
\begin{frame}[c]
  ...
  \begin{columns}[c]
    ...
    \begin{column}{0.35\textwidth}
      \uncover<1->{\includegraphics[width=\textwidth]{figures/front-cover.png}}
    \end{column}
  \end{columns}

\end{frame}

\section{Screening Rules (Papers I--III)}
...
\begin{frame}[c]
  ...
  \begin{block}{Motivation}
    Fitting models to sets of millions or billions of features is computationally costly, and
    exacerbated by the fact we often need to do cross-validation.
  \end{block}

  \pause

  \begin{block}{Basic Idea of Screening}
    \begin{itemize}
      ...
      \item If we could estimate the model's support \textbf{before} fitting the model, then we would
            save a lot of time.
    \end{itemize}
  \end{block}
  ...
  \begin{block}{General Outline}
    \begin{enumerate}
      \item Estimate how likely it is that a feature is in the support set.
      \item If unlikely, discard it and fit a smaller model.
      \item If we are wrong, just refit the model with missing features added.
    \end{enumerate}
  \end{block}
\end{frame}
...
\begin{frame}[c]
  \begin{figure}[htpb]
    \centering
    \includegraphics[]{figures/regularization-path.pdf}
    \caption{%
      The regularization path for the lasso. The path is piecewise linear.
    }
  \end{figure}
...
\begin{frame}[c]
  \begin{figure}[htpb]
    \centering
    \uncover<1->{\includegraphics{figures/sparsity-path-1.pdf}}\hfill%
    \uncover<2->{\includegraphics{figures/sparsity-path-2.pdf}}\hfill%
    \uncover<3->{\includegraphics{figures/sparsity-path-3.pdf}}
    \uncover<1->{%
      \caption{%
        Non-zero coefficients along the path, ordered by the time they enter the model. The
  ...
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{block}{Optimality Condition}
        \(\bm{\beta}\) is optimal if \(\boldsymbol{0} \in \nabla g(\bm{\beta}) + \partial h(\vec{\beta}),\) where \(\partial h (\vec{\beta})\) is the subdifferential of the penalty term \(h\).
      \end{block}

      \uncover<2->{
        \begin{exampleblock}{Example: Lasso}
          \[
            \partial h (\bm{\beta})_j =
            \begin{cases}
              \{
            ...
            \end{cases}
          \]
          So if \(|\nabla g(\hat{\bm{\beta}})_j| < \lambda,\) \alert{then \(\hat{\beta}_j = 0\)}.
        \end{exampleblock}
      }

      \uncover<3->{
        \begin{block}{Screening Rule Template}
          \begin{enumerate}
            \item Replace \(\nabla g(\hat{\bm{\beta}})\) with an estimate \(\tilde{\nabla} g(\vec{\beta})\).
            \item If \(|\tilde{\nabla} g(\vec{\beta})_j| < \lambda,\) discard feature \(j\).
          \end{enumerate}
        \end{block}
      }
...
\begin{frame}[c]
  \begin{figure}[htpb]
    ...
    \caption{%
      Coefficients and correlations (\(\bm{c}\)) along the lasso path. If
      the gradient of any given coefficient is smaller than the penalty (dotted line), then
      the coefficient has to be zero.
    }
  \end{figure}
\end{frame}
...
\begin{frame}[c]
  \frametitle{Strong Rule for SLOPE (Paper I)}
  ...
  \begin{columns}[T]
    \begin{column}{0.45\textwidth}
      ...
      In our paper\footnote[frame]{\fullcite{larsson2020b}}, we derive the subdifferential for
      SLOPE/OWL and present an efficient algorithm for using it to screen features.
    \end{column}
    \begin{column}{0.5\textwidth}

      \begin{center}
        \frame{\includegraphics[height=3cm]{figures/gosia.jpeg}}\hfill%
        \frame{\includegraphics[height=3cm]{figures/jonas.jpg}}
      \end{center}
    \end{column}
  \end{columns}
...
\begin{frame}[c]
  \begin{figure}[htpb]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/paper1-efficiency-real.pdf}
    \caption{%
      Effectiveness of the strong screening rule for SLOPE on some real data sets.
    }
  \end{figure}
\end{frame}
...
\begin{frame}[c]
  \frametitle{Look-Ahead Screening (Paper II)}

  \begin{itemize}[<+->]
    \item There's nothing special about the current step's \(\lambda\) in a screening rule, so why
          not look further ahead?\footnote{\fullcite{larsson2021}}
    \item The problem is that the strong rule is heuristic.
    \item But there are \alert{safe rules}, that guarantee that we do not make a mistake.
  \end{itemize}
  ...
  \begin{figure}[htpb]
    \centering
    \includegraphics{figures/paper2-casestudy.pdf}
    \caption{%
      Lasso path for a sample of features from the leukemia data set. The squares
...
%   \begin{figure}[htpb]
%     \centering
%     \includegraphics[width=0.8\textwidth]{figures/paper2-simulated-data.pdf}
%     \caption{%
...
\begin{frame}
  \frametitle{The Hessian Screening Rule (Paper III)}
  % \begin{columns}[c]
  % \begin{column}{0.45\textwidth}

  A screening rule for the lasso that uses second-order information to obtain better
  estimates of future gradients.\footnote[frame]{\fullcite{larsson2022b}}

  \pause

  \begin{block}{Improved Screening}
    Uses information about the gradient of the gradients along the path, which is
    less conservative than the strong rule.
  \end{block}

  \pause

  \begin{block}{Better Warm Starts}
    Allows also to estimate the entire coefficient vector for the next step.
  \end{block}
  ...
  %   \end{align*}
  %   solution is \alert{piecewise linear}.\medskip
  %
  %   And if active set is constant in \([\lambda_k,\lambda_{k+1}]\), we can express the solution
  %   in closed form.
  % \end{column}
  % \begin{column}{0.45\textwidth}
  %   \centering
...
%   \begin{figure}
%     \centering
%     \includegraphics[width=0.8\textwidth]{figures/paper3-simulateddata-efficiency}
%     \caption{%
...
\begin{frame}[c]
  ...
  \begin{columns}[c]
    \begin{column}{0.45\textwidth}
      \begin{itemize}[<+->]
        \item SLOPE has appealing properties but algorithms for solving SLOPE are\footnote{Used to be!}
              slow (relative to the lasso)
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.51\textwidth}
      \only<1->{%
        \begin{figure}[htpb]
          \centering
          \includegraphics[width=0.9\textwidth]{figures/paper5-naive-cd-stuck.pdf}
...
\begin{frame}[c]
  ...
  Our algorithm is a combination of coordinate descent and proximal gradient
  descent:\footnote{\fullcite{larsson2023}}
...
\begin{frame}
  \begin{figure}[htpb]
    \centering
    \includegraphics[scale=0.70]{figures/paper5-simulated-legend.pdf}
    \includegraphics[scale=0.70]{figures/paper5-simulated.pdf}
    \caption{%
...
\begin{frame}[c]
  \frametitle{The SLOPE Package(s)}

  \begin{columns}[T]
    \begin{column}{0.7\textwidth}
      \begin{block}{LibSLOPE}
        \begin{itemize}
          \item \texttt{C++} library for solving SLOPE using the hybrid coordinate method
          \item Parallelized fitting and cross-validation
          \item Algorithmic improvements to SLOPE thresholding and cluster handling for sparse data
        \end{itemize}
      \end{block}
    \end{column}
    \begin{column}{0.3\textwidth}
      \hfill\includegraphics[width=0.8\textwidth]{figures/slope-logo.png}
    \end{column}
  \end{columns}
  ...
  \begin{block}{Packages}
    \begin{center}
      \begin{tabular}{lll}
        ...
        \midrule
        R        & SLOPE    & \url{https://jolars.github.io/SLOPE}    \\
        Python   & sortedl1 & \url{https://jolars.github.io/sortedl1} \\
        C++      & libslope & \url{https://jolars.github.io/libslope} \\
        \bottomrule
      \end{tabular}
    \end{center}
...
\section{The Choice of Normalization Affects Shrinkage in Regularized Regression}
...
\begin{frame}[t]
  ...
  \begin{equation*}
    \hat{\bm{\beta}} = \operatorname*{arg\,min}_{\beta \in \mathbb{R}^p} \bigg( \frac{1}{2} \lVert \vec y - \mat{X}\vec{\beta} \rVert^2_2  + \underbrace{\lambda_1 \lVert \vec\beta \rVert_1}_\text{lasso} + \underbrace{\frac{\lambda_2}{2}\lVert \vec \beta \rVert_2^2}_\text{ridge}\bigg)
  \end{equation*}
...
\begin{frame}[c]
  \begin{figure}[htpb]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/paper6-realdata_paths.pdf}
    \caption{%
...
\begin{frame}[c]
  ...
  \begin{block}{Motivation}
    \begin{itemize}[<+->]
      \item Normalization matters, yet there is almost no research on this topic.
      \item Everyone agrees you need to normalize, but how to do so is usually motivated by being
            ``standard''.
      \item Documentation for popular software advocate different normalization strategies when data is
            sparse.
      \item Approach depends on field: statisticians standardize, signal processors use $\ell_1$
            normalization, machine learners scale to $[0, 1]$ or $[-1, 1]$.)
    \end{itemize}
  \end{block}

  \pause

  \begin{block}{Our Paper: Focus on Binary Features}
    In our paper, we focus on binary features particularly with respect to \alert{class
      balance}.
  \end{block}
...
\begin{frame}[c]
  ...
  \begin{columns}
    \begin{column}{0.55\textwidth}
      ...
      \[
        \tilde{\bm{X}}^\intercal \tilde{\bm{X}} = \diag(\tilde{\vec{x}}_1^\intercal \tilde{\vec{x}}_1, \dots, \tilde{\vec{x}}_p^\intercal \tilde{\vec{x}}_p),
      \]
      then there is:\footnote<2->[frame]{We have also assumed that the features are mean-centered
        here.}:
      \begin{equation*}
...
\begin{frame}[c]
  ...
  \begin{itemize}[<+->]
    ...
    \item Suggests the parametrization
          ...
          \]
          which we will rely on for the sequel.
...
\begin{frame}[c]
  \frametitle{The Paper\footnote{\fullcite{larsson2025}}}

  \begin{columns}[T]
    \begin{column}{0.47\textwidth}
      \begin{block}{Parameterization}
        We introduce a scaling parameterization
        \[
          s_j = (q - q^2)^\delta ,\quad \delta \geq 0.
        \]
        so that \(\delta = 0\) corresponds to no scaling, \(\delta = 1/2\) standardization, and
        \(\delta = 1\) to variance scaling.
      \end{block}

      \pause

      \begin{block}{Class Balance}
        We focus on binary features and the effect of class balance on the elastic net.
      \end{block}
    ...
    \begin{column}{0.47\textwidth}
      \begin{block}{Bias and Variance}
        We introduce normal noise and study bias, variance, and probability of selection.
      \end{block}
      \pause

      \begin{block}{Mixed Data (Not Covered Here)}
        We consider a mix of binary and continuous features.
      \end{block}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[c]
  ...
  Assuming \(\mat{X}\) is fixed and noise is normal, we can compute the probability of
  selection.
  % \[
...
\begin{frame}[c]
  ...
  \begin{block}{Screening Rules (I--III)}
    Screening rules for SLOPE and the lasso.
  \end{block}

  \pause

  \begin{block}{Benchopt (IV)}
    Benchmarking optimization methods.
  \end{block}

  \pause

  \begin{block}{Coordinate Descent for SLOPE (V)}
    A hybrid coordinate descent method for SLOPE.
  \end{block}

  \pause

  \begin{block}{Normalization and Regularization (VI)}
    The interplay between normalization and regularization.
  \end{block}
...
\begin{frame}[c]
  \begin{figure}[htpb]
    \centering
    \includegraphics[width=0.90\textwidth]{figures/paper1-performance.pdf}
    \caption{%
...
\begin{frame}
  \begin{figure}
    \centering
    \includegraphics[scale=0.6]{figures/paper5-real_legend.pdf}
    \includegraphics[scale=0.6]{figures/paper5-real.pdf}
    \caption{%
...
\begin{frame}[c]
  \begin{figure}
    \centering
    \includegraphics[width=0.90\textwidth]{figures/paper6-bias-var-onedim.pdf}
    \caption{%
